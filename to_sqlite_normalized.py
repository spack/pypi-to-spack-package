...
with open(json_file, "rb") as f:
    ...
    while True:
        batch = [x for _ in range(batch_size) if (x := json.loads(f.readline()))]
        ...
        name_version_to_id = {(name, version): id for id, name, version in result}
        ...
        for data in batch:
            ...
            try:
                entries.extend(
                    (id, *requirement_parts(entry)) for entry in data["requires_dist"]
                )
            except InvalidRequirement as e:
                # Just delete the entry, cause it's broken.
                print(
                    f"Deleting {data['name']} {data['version']} because it is broken: {e}"
                )
